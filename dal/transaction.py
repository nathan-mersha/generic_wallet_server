...
class TransactionModelDAL:
    ...
    async def fulfil_transaction(self, transactionModel: TransactionModel):
        with self.client.start_session() as session:
            with session.start_transaction():
                # updating sender's account
                sender_query = {"id" : transactionModel.from_user["id"]}
                ...
                transaction_query = {"id" : transactionModel.id}
                update_data = {"trnType" : "transfer"}
                return self.update(transaction_query, update_data)
